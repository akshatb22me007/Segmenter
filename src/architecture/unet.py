...
class UNet(nn.Module):
    def __init__(self,model_depth=5,low_dim=64,high_dim=1024,in_channel=3,n_class=2):
    ...
    def build_encoder(self):
        ...
        for i in reversed(range(self.model_depth)):
            ...
            self.encoder_blocks.append(Encoder_Block(in_channel=temp,initial_filter=initial_filter,num_layers=2).to(device))         
            temp=initial_filter
    ...
    def build_decoder(self):
        ...
        for i in reversed(range(1,self.model_depth)):
            ...
            self.upsample_blocks.append(nn.ConvTranspose2d(initial_filter,initial_filter//2,2,2).to(device))
            self.decoder_blocks.append(Decoder_Block(initial_filter=initial_filter,num_layers=2).to(device))
